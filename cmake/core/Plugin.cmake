...
macro(calm_plugins)
    ...
    foreach (_plugin ${ARGN})
        if (${_need_var_name})
            ...
            if (DEFINED ${_plugin})
                if (${${_plugin}})
                    message(STATUS "Enable plugin ${_saved_plugin}")
                    set(_plugin ${_saved_plugin})
                    set(_saved_plugin "")
                else()
                    message(STATUS "The plugin `${_saved_plugin}` is disabled because ${_plugin} is ${${_plugin}}")
                    continue()
                endif()
            else()
                message(WARNING "Variable ${_plugin} is not defined. It controls the enablement of the plugin `${_saved_plugin}`. The plugin will be disabled.")
                continue()
            endif()
        endif()
        if (_plugin STREQUAL "ENABLED_BY")
            set(_need_var_name ON)
            continue()
        else()
            if (NOT _saved_plugin)
                #message(STATUS "1. _plugin = ${_plugin}")
                set(_saved_plugin ${_plugin})
                continue()
            else()
                #message(STATUS "2. _plugin = ${_saved_plugin}")
                set(_tmp_var ${_plugin})
                set(_plugin "${_saved_plugin}")
                set(_saved_plugin ${_tmp_var})
            endif()
        endif()
        message(STATUS "_plugin = ${_plugin}")
