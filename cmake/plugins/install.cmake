...
function(_plugin_install_apply _target)
    ...
    if(${_type} MATCHES "(.*)_LIBRARY")
        set_target_properties(fmt PROPERTIES EXPORT_NAME MyInstall)
...
function(_amend_link_libraries _target _dependencies)
    ...
    endforeach()
    message(STATUS "_rebased_libraries = ${_rebased_libraries} (type = ${_type})")
...
function(_install_library _target _namespace)
    ...
    if (_type STREQUAL INTERFACE_LIBRARY)
        ...
        get_target_property(_dependencies ${_target} LINK_LIBRARIES)
    endif()
    ...
    foreach(_dependency ${_targets})
        ...
        message(STATUS "[${_dependency}][${_type}] _includes = ${_includes}")
        _amend_include_directories(${_dependency} "${_includes}")
        if (_type STREQUAL INTERFACE_LIBRARY)
            get_target_property(_includes ${_dependency} INTERFACE_INCLUDE_DIRECTORIES)
        else()
            get_target_property(_includes ${_dependency} INCLUDE_DIRECTORIES)
        endif()
        message(STATUS "[${_dependency}] _includes = ${_includes}")
    endforeach()
    _amend_link_libraries(${_target} "${_targets}")
    message(STATUS "_targets = ${_targets}")
    install(TARGETS ${_target} fmt boost_filesystem
